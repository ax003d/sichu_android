...
public class SichuContentProvider extends ContentProvider {
	...
	private static final String BOOKOWNS_TABLE_NAME = "bookowns";
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + BOOKS_TABLE_NAME + " (" + Books._ID
			...
			db.execSQL("CREATE TABLE " + BOOKOWNS_TABLE_NAME + " ("
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			db.execSQL("DROP TABLE IF EXISTS " + BOOKOWNS_TABLE_NAME);
			onCreate(db);
		}
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch ( URI_MATCHER.match(uri) ) {
		case BOOKS:
			row_id = db.insert(BOOKS_TABLE_NAME, Books.GUID, values);
			if ( row_id > 0 ) {
			...
			break;
		case BOOKOWNS:
			row_id = db.insert(BOOKOWNS_TABLE_NAME, BookOwns.GUID, values);
			if ( row_id > 0 ) {
	...
	static {
		...
		URI_MATCHER.addURI(AUTHORITY, BOOKOWNS_TABLE_NAME + "/owner/#",
				BOOKOWNS_BY_OWNER);
		URI_MATCHER.addURI(AUTHORITY, BOOKS_TABLE_NAME, BOOKS);
		URI_MATCHER.addURI(AUTHORITY, BOOKOWNS_TABLE_NAME, BOOKOWNS);
		URI_MATCHER.addURI(AUTHORITY, BOOKS_TABLE_NAME + "/guid/#", BOOK_BY_GUID);
		URI_MATCHER.addURI(AUTHORITY, BOOKOWNS_TABLE_NAME + "/guid/#", BOOKOWN_BY_GUID);

		bookownsWithBookProjectionMap = new HashMap<String, String>();
		bookownsWithBookProjectionMap.put(BOOKOWNS_TABLE_NAME  + "." + BookOwns.GUID, 
				BOOKOWNS_TABLE_NAME  + "." + BookOwns.GUID);		
